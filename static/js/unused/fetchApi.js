...
import { showErrorToast, showSuccessToast } from '../script.js'
...
export async function fetchDataAndRender(actionId, datetimeset) {
  try {
    ...
    if (actionId === 'top-center') {
      ...
      showErrorToast('ดึงข้อมูลไม่สําเร็จ ลืมใส่เวลา หรือไม่มีวันที่นั้นๆ');
      }
    }
    ...
    data_chart = '';
      ...
      `;
      showErrorToast('ไม่มีข้อมูล');
    }
}
const toggle = document.getElementById("toggle");
const webtag2 = document.getElementById("web-tag2");
const closeModal = document.getElementById("closeModal");
const modal = document.getElementById("myModal");
const webcom1 = document.getElementById("webcom1_i");
const webcom2 = document.getElementById("webcom2_i");
...
export function condition(actionId) {
    ...
    if (actionId === "total-month") {
      //ควบคุมเปิดปิดข้อมูลช่องสอง
      toggle.addEventListener('change', () => {
        if (toggle.checked) {
          console.log('toggle checkedd')
          webtag2.classList.remove('hidden');
        } else {
          console.log('ไม่เจอ')
          webtag2.classList.add('hidden');
        }
      })
      ...
      okbutton.addEventListener("click", () => {
        const str = webcom1.value;
        const str2 = webcom2.value;
        webcom = [str,str2];
